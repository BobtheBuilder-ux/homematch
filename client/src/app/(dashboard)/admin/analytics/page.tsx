...
import { useGetAdminAnalyticsQuery } from "@/state/api";
import {
  BarChart,
  Bar,
  XAxis,
...
const Analytics = () => {
  ...
  if (isError || !analytics) return <div>Error loading analytics</div>;
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">
              +{analytics.propertiesGrowth}% from last month
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">
              +{analytics.applicationsGrowth}% from last month
            </p>
          </CardContent>
        </Card>
            ...
            <ResponsiveContainer width="100%" height={300}>
              <PieChart>
                <Pie
                  ...
                  cy="50%"
                  labelLine={false}
                  label={({ name, percent }) => name && percent !== undefined ? `${name} ${(percent * 100).toFixed(0)}%` : `${name} 0%`}
                  outerRadius={80}
                  fill="#8884d8"
                  dataKey="count"
                >
                  {analytics.propertyTypes.map((entry: any, index: number) => (
